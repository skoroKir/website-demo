// const uploaderBtn1 = document.querySelector('.js-open-uploaderInsertQuote_1') //declated in control.html
...
function initCropper (image) {
    ...
    if (window.uploader.button == 'img_0'){
        viewPortMeasurments = {
            width: 350,
            height: 350,
            type: 'circle'
    ...
    if (window.cropper) { 
        window.cropper.result('blob').then(async function(blob) {
            ...
            let data = new FormData(),
                ...
                filename = `${uploadLocation}${uploader.button}_${InsertsStartSessionID}_${uploader.file.name}`
            ...
            let result = await fetch('../../uploader/uploader.php', {method:"POST", body: data})
